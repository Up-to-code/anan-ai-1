...
test("userProfiles upsert creates new profile", async () => {
  ...
  const id = await t.mutation(api.features.users.profiles.upsert, {
    userId: "test-user-1",
    salary: 60000,
    employment: "employed",
  });
  ...
  const profile = await t.query(api.features.users.profiles.getByUserId, {
    userId: "test-user-1",
  });
...
test("userProfiles upsert patches existing profile", async () => {
  const t = convexTest(schema, modules);
  await t.mutation(api.features.users.profiles.upsert, {
    userId: "test-user-2",
    salary: 50000,
    kids: 2,
  });
